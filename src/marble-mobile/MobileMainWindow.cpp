...
void MainWindow::showDownloadRegionDialog()
{
    if ( !m_downloadRegionDialog ) {
        ...
        connect( m_downloadRegionDialog, SIGNAL(hidden()), SLOT(disconnectDownloadRegionDialog()));
    }
...
{
    disconnect( m_marbleWidget, SIGNAL(visibleLatLonAltBoxChanged(GeoDataLatLonAltBox)),
                m_downloadRegionDialog, SLOT(setVisibleLatLonAltBox(GeoDataLatLonAltBox)));
